...
You could also use the tab key to indent things but I use spaces. Once that is added lets open the file in the browser again and see if our button works.Now if you press the button it should change the text to the answer but if you try pressing it again the card doesn't flip back. 

So How can we make the card flip? Let's make a variable to keep track of if the card is face up. Let's start by adding a couple new lines at the begining of our js file. Then we will make a variable that will keep track of if you card is face up:
```
Let faceup = true;
...
```
Then down in our function we will add a if statements where the computer will check if the variable is true and then change the text and turn faceup to false.
```
function flipcard(){
  if(faceup){
        document.getElementById("Flashcard").innerHTML = "How to save a life";
...
```
Then we will write an else statment to change the text if the card is bottom up:
```
function flipcard {
  if {
    ...
...
```
Okay, now lets try that out in the browser.

Next lets make the hint button work. First in the html file lets add at the very bottom just below the body tag a empty paragraph tag:
```
</div>
<p id="hint"></p>
</body>
```
Then in our js file add new function to the end of the file.
```
function giveHint() {
  document.getElementById("hint").innerHTML = "Go to wikipedia and find the band The Fray";
}
```
And open the page in the browser. For extra credit see if you can do the following:

- Change the color of the hint to something more readable like white or wheat.
- Make the hint button text change to "hide hint" when the button is clicked.
- Make the hint toggle between shown and hiden (Have the text change on the button too).
- Make the hint caption centered below the card and buttons.
